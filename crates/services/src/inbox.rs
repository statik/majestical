//! `maj inbox process` compute: one converging pass over a shared drop
//! folder. Contribution = a subfolder with a `contribution.json` manifest
//! (the documented integration point for the share-sheet Shortcut and
//! future iOS app). Manifest-less entries (a subfolder with no manifest, or
//! a bare top-level file) triage instead: once quiescent (no contained
//! mtime younger than [`quiescence_ms`]), they ingest to a triage target,
//! tagged `source/inbox`, no contributor identity claimed. Reuses
//! `crate::ingest`'s verified-copy pipeline end to end. Manifest parsing and
//! the read-only validation gates live in `crate::inbox_manifest`. Moved
//! from `crates/cli/src/inbox_cmd.rs`; rendering (`print_report` and its
//! JSON/text variants) stays in the CLI.
...
use crate::ingest::{ExecuteIngest, known_assets_from_projection, render_ingest_subdir};
...
/// `maj inbox process`'s request: which inbox to sweep, where verified
/// copies land, the PARA target for manifest-less drops (required once any
/// quiescent manifest-less item is present — see [`process_manifest_less`]),
/// and whether a successfully ingested contribution stays in place instead
/// of moving to `.processed/`.
...
/// Catalog root + request, threaded through every contribution — bundled so
/// `process_contribution`/`ingest_contribution` stay within the house
/// 5-positional-parameter limit. `inbox_key` is computed once here (not
/// re-derived per contribution) so every marker lookup and record in this
/// pass agrees on the same identity.
struct InboxCtx<'a> {
    catalog: &'a Path,
...
fn contribution_fingerprint(dir: &Path, manifest: Option<&ContributionManifest>) -> String {
    ...
    let (mtime, size) = std::fs::symlink_metadata(dir.join(MANIFEST_NAME))
        .map_or((0, 0), |m| (mtime_ms_of(&m), m.len()));
...
/// One row of `maj inbox process`'s report: a contribution folder, a
/// manifest-less folder, `(loose files)`, or `(manifest-less)`, paired with
/// what happened to it.
#[derive(Debug, serde::Serialize)]
pub struct ContribRow {
    pub name: String,
    pub outcome: ContribOutcome,
}

/// Everything `maj inbox process` renders: every row from this pass, in the
...
impl InboxOutcome {
    /// A fresh failure (this run) fails the pass; a previously RECORDED
    /// failure is a notice only. Moved verbatim (as a boolean; the CLI
    /// still walks `rows` itself to print each fresh failure's detail to
    /// stderr) from `crates/cli/src/inbox_cmd.rs::print_report`'s
    /// `any_failed` loop.
    #[must_use]
    pub fn overall_failed(&self) -> bool {
        self.rows.iter().any(|row| {
            matches!(
                row.outcome,
                ContribOutcome::Failed { .. } | ContribOutcome::PartlyIngested { .. }
            )
        })
    }
}
...
fn newest_mtime_ms(path: &Path) -> Option<u64> {
    ...
    if !top.is_dir() {
        return Some(mtime_ms_of(&top));
    }
    let mut newest = mtime_ms_of(&top);
...
fn is_quiescent(path: &Path, window_ms: u64) -> bool {
    ...
    physical_now_ms().saturating_sub(newest) >= window_ms
}
...
/// `maj inbox process`: one converging pass over `req.inbox`. Every
/// manifested subfolder is validated, verified-ingested, tagged with
/// provenance, and (unless `req.keep`) moved to `.processed/`.
/// Manifest-less entries (a subfolder with no `contribution.json`, or a
/// bare top-level file) triage instead — see [`process_manifest_less`].
/// Moved from `crates/cli/src/inbox_cmd.rs::cmd_inbox_process`.
///
/// # Errors
/// Returns an error if `req.inbox` isn't a directory or can't be
/// canonicalized, or if loading or storing the failure-marker store fails.
/// Reading the inbox directory itself is the one per-contribution-loop
/// failure that still aborts the pass (an operator-facing I/O problem, not
/// any one contribution's fault) — markers recorded by contributions
/// processed earlier in the same pass are always persisted first, so that
/// failure never discards them. Per-contribution failures are reported
/// inside the returned outcome, never as an `Err` — see
/// [`InboxOutcome::overall_failed`].
pub fn process(
    app: &mut FsApp,
    catalog: &Path,
    req: &ProcessRequest,
) -> Result<InboxOutcome, ServiceError> {
    process_impl(app, catalog, req).map_err(ServiceError::from)
}

fn process_impl(app: &mut FsApp, catalog: &Path, req: &ProcessRequest) -> Result<InboxOutcome> {
    anyhow::ensure!(
        req.inbox.is_dir(),
...
/// function can store markers before propagating any error this returns —
/// a pass-fatal I/O error must never discard markers other contributions
/// already recorded earlier in the same pass. A manifest that fails to load
/// (malformed JSON, a validation refusal) is itself recorded under a
/// manifest-only fingerprint, same as any other failure, so a hand-broken
/// manifest converges to a notice on the next pass instead of failing
/// forever.
fn run_pass(
    app: &mut FsApp,
    ctx: &InboxCtx<'_>,
    markers: &mut FailureMarkers,
) -> Result<Vec<ContribRow>> {
    let mut report = Vec::new();
    let mut triage_dirs = Vec::new();
    for path in list_contribution_dirs(&ctx.req.inbox)? {
        let name = display_name(&path);
        match load_manifest(&path) {
            Ok(Some(manifest)) => {
                let outcome = process_contribution(app, ctx, &path, &manifest, markers)?;
                report.push(ContribRow { name, outcome });
            }
    ...
    let loose_files = list_loose_files(&ctx.req.inbox)?;
...
/// [`crate::ingest::resolve_ingest_node`]'s generic "see `maj para list`"
/// (which doesn't tell the operator what to do next) — distinguishing the
/// two ways "not active" happens matters: a target that was never created
/// needs `maj para add`, but a target that exists and is merely archived
/// would get a DUPLICATE node from that same advice, so it gets its own
/// message naming un-archiving instead.
fn resolve_contribution_node(
    projection: &Projection,
    para: &str,
) -> Result<(String, ParaKind, String)> {
    if let Some((kind_str, name)) = para.split_once('/')
        && let Ok(kind) = crate::para::parse_kind(kind_str)
    {
    ...
    crate::ingest::resolve_ingest_node(projection, para)
}
...
fn triage_loose_files_ingest(
    ...
    let ingest_plan =
        plan::plan_source_filtered(&ctx.req.inbox, &known, plan::DedupeMode::Skip, &filter)
            .with_context(|| format!("planning inbox {}", ctx.req.inbox.display()))?;
    run_shared_ingest(
        app,
        ctx,
        &ingest_plan,
...
/// lines, not two identical, unattributed ones.
#[derive(Clone, Copy)]
struct IngestSite<'a> {
    probe_dir: &'a Path,
    row_name: &'a str,
...
fn run_shared_ingest(
    ...
    let (vol_id, vol_label) = resolve_volume(site.probe_dir, None);
    ...
    // one layout.
    let subdir = render_ingest_subdir(kind, &name, "{date}/{source-label}", &vol_label)?;
    let run = crate::ingest::run_ingest(
        app,
        ctx.catalog,
        &ExecuteIngest {
            plan: ingest_plan,
...
/// — quiesce before they're touched, then triage to `ctx.req.triage_target`
...
fn process_manifest_less(
    ...
    let Some(triage) = ctx.req.triage_target.as_deref() else {
        report.push(ContribRow {
            name: "(manifest-less)".to_string(),
            outcome: ContribOutcome::Failed {
    ...
    if !quiescent_files.is_empty() {
        ...
        report.push(ContribRow {
            name: LOOSE_FILES_ROW.to_string(),
            outcome,
        });
    }
    Ok(())
...
fn push_waiting_row(
    ...
    report.push(ContribRow {
        name: "(manifest-less)".to_string(),
        outcome: ContribOutcome::Waiting { reasons },
    });
}
...
fn process_triage_dir(
    ...
    if !ctx.req.keep {
        move_into_processed(&ctx.req.inbox, dir)?;
    }
    Ok(contrib)
}
    ...
    if !ctx.req.keep {
        ...
        for file in files {
            if drained.contains(display_name(file).as_str()) {
                move_into_processed(&ctx.req.inbox, file)?;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn overall_failed_is_true_when_a_row_freshly_failed() {
    ...
    #[test]
    fn overall_failed_is_false_for_ingested_waiting_and_recorded_failure_rows() {
        let outcome = InboxOutcome {
            rows: vec![
                ContribRow {
                    name: "drop-1".to_string(),
                    outcome: ContribOutcome::Ingested {
                        placed: 1,
                ...
                ContribRow {
                    ...
                    outcome: ContribOutcome::Waiting {
                        reasons: vec!["still uploading".to_string()],
                    },
                },
                ContribRow {
                    name: "drop-3".to_string(),
                    outcome: ContribOutcome::RecordedFailure {
                        reason: "already recorded".to_string(),
                    },
                },
            ],
        };
    ...
    #[test]
    fn overall_failed_is_true_for_a_partly_ingested_row() {
        let outcome = InboxOutcome {
            rows: vec![ContribRow {
                name: "(loose files)".to_string(),
                outcome: ContribOutcome::PartlyIngested {
                    placed: 1,
                    skipped_duplicates: 0,
                    failed: 1,
                },
    ...
    #[test]
    fn process_of_a_nonexistent_inbox_errors() {
        let dir = tempfile::tempdir().expect("tempdir");
        ...
        assert!(err.to_string().contains("inbox must be a directory"));
    }

    #[test]
    fn process_of_an_empty_inbox_reports_no_rows() {
        let dir = tempfile::tempdir().expect("tempdir");
        let root = dir.path().join("cat");
        let mut app = FsApp::init(&root, "m1", "m1").expect("init");
