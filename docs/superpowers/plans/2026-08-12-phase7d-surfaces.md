...
- [ ] **Step 1: failing component tests**:
  ...
  **AMENDED — no per-node asset counts: WIRE GAP.** `ParaNodeRow` carries id, kind, name and archived and nothing else, so the mockup's "1,204" (and its "materialized at …" line) cannot be drawn. Rows and the detail card render without them; both need a field on the Rust row first.
  ...
  **AMENDED — the roots come from a new `list_mounted_roots` command, not from the dry run**: deriving them from the archive dry-run's own outcome is circular (the dry run takes roots as its input), and nothing in the catalog records where a node was materialized. Added one read command `list_mounted_roots` → `Vec<MountedRoot { volume, label, path }>` over `majestical_services::volume_identity::mounted_volumes()` (a `BTreeMap<volume id, mount point>`; the label is the mount's last path component, `ROOT_LABEL` for `/`), wired like every other command (impl + one-liner + test + fixture + `api.ts`'s `listMountedRoots`). The modal previews against the volumes mounted right now. Two further divergences from this task's file list: `nearDuplicates` lives in its own `organize-tags.ts` (the `browse-paths.ts` precedent for a pure helper), and the modal is its own `ArchiveModal.svelte` — oxlint's `max-lines` counts a component's `<script>` block, and one component holding both columns and the modal is over it.
...
**Files:**
- Modify: `crates/cli/tests/services_parity.rs` (rows for the new verbs, and delete Task 4's temporary keyframe-images normalization — see Step 1), `apps/desktop/src-tauri/tests/tauri_parity.rs` (rows for browse/tags/para-file/unfinished verbs)
