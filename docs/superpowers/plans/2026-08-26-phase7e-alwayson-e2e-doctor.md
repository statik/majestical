...
| `blob_residue` | the truncated-tail scan reports zero residue | Reuse the detection in `crates/services/src/index/heal.rs` in report-only form (find the entry point with `rg -n "pub fn" crates/services/src/index/heal.rs`); Warn with count + remedy naming the heal verb. Skipped as Warn when no catalog. |
