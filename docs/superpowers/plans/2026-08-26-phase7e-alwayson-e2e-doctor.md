...
> **AMENDED (2026-09-14, Task 14 as-built; deviations from the approved
> tray mockup, each forced by the platform):** (a) the attention state is
> a badge dot on the template icon (`icons/tray/attention.png`), not an
> amber tint — macOS menu-bar template icons are alpha-only; (b) only the
> 22×22 @1x PNGs are loaded at runtime: tray-icon 0.24 builds the NSImage
> from raw pixels 1:1 in points, so the committed @2x files wait for a
> HiDPI pass; icons are generated by the root `just tray-icons` recipe
> (ImageMagick) and committed; (c) Quit is an immediate `app.exit(0)`,
> not "waits for the current batch" — SQLite keeps an aborted batch
> atomic and the scheduler redoes it later; (d) Cmd+Q from the default
> macOS app menu also exits, so Quit is not the only way out; (e) a left
> click on the icon opens the menu only — macOS menu tracking swallows
> the mouse-up, so no click-to-open-window handler exists; "Open
> Majestical" is the way in. (f) e2e coverage gap: window-close
> hide-to-tray is untested — the WebDriver session cannot survive a
> hidden window.
